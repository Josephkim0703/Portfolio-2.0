...
function StartPage() {
  const [wallpaper, setWallpaper] = useState(
    "./public/assets/wallpaper/default.png"
  );
  const [headerId, setHeaderId] = useState("");
  const [location, setLocation] = useState(0);
  const [index, setIndex] = useState(0);
  const [popupSetting, setPopup] = useState("")

  const [hide, setHide] = useState(Array(10).fill(false));
  const [opacity, setOpacity] = useState(Array(10).fill(0));
  const [active, setActive] = useState(Array(10).fill(null));
  ...
  return (
    <>
      <div id="screen_Border">
        <Header
          grabHeaderId={handleGrabHeaderId}
          hide={updateHide}
          index={setIndex}
        />
        {hide[0] && (
        ...
        <Taskbar setTab={setTab} setActive={setActive} setPopup={setPopup}/>
